...
let pp_program prog out_channel =
  ...
  | Var(x) -> x
  | Unop (u, e) ->
    sprintf "(%s%s)" (pp_unop u) (pp_expression e)
  | Binop(op, e1, e2) ->
    sprintf "(%s%s%s)" (pp_expression e1) (pp_binop op) (pp_expression e2)
  | Call(f, args) ->
    sprintf "%s(%s)" (pp_func f) (pp_args args)
  | Seq (s, e) ->
  ...
  and pp_func: func -> string = function
  | FName name -> name
  | FPointer e -> pp_expression e
  ...
  and pp_instruction = function
    ...
    | Write (e1, e2) ->
      print "%s := %s;" (pp_expression e1) (pp_expression e2)
